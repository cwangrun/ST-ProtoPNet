...
from util.preprocess import mean, std, preprocess_input_function, undo_preprocess_input_function
import argparse
...
test_image_dir = './local_analysis/class148/'
test_image_name = 'Green_Tailed_Towhee_0003_797414.jpg'
test_image_label = 147
prototype_name = 'support'
# prototype_name = 'trivial'


test_image_path = os.path.join(test_image_dir, test_image_name)

# load the model
load_model_dir = './saved_models/20230820_1653/resnet34/001'
load_model_name = '20_8push0.8186.pth'

model_base_architecture = load_model_dir.split('/')[-2]
experiment_run = load_model_dir.split('/')[-1]

save_analysis_path = os.path.join(test_image_dir, model_base_architecture, experiment_run, load_model_name, prototype_name)
...
ppnet = model.construct_STProtoPNet(base_architecture=model_base_architecture,
...
from settings_CUB_DOG import test_dir
...
if prototype_name == 'support':
    load_img_dir = os.path.join(load_model_dir, 'imgsupport')
else:
    load_img_dir = os.path.join(load_model_dir, 'imgtrivial')
...
if prototype_name == 'support':
    prototype_max_connection = torch.argmax(ppnet.last_layer_support.weight, dim=0)
else:
    prototype_max_connection = torch.argmax(ppnet.last_layer_trivial.weight, dim=0)
...
def save_prototype_self_activation(fname, epoch, index):
    p_img = plt.imread(os.path.join(load_img_dir, 'epoch-' + str(epoch),
                                    str(index) + 'prototype-img-original_with_self_act' + '.png'))
    # plt.axis('off')
    plt.imsave(fname, p_img)
...
def save_prototype_original_img_with_bbox(fname, epoch, index,
    ...
    p_img_bgr = cv2.imread(
        os.path.join(load_img_dir, 'epoch-' + str(epoch), str(index) + 'prototype-img-original' + '.png'))
...
output, max_similarities, similarity_maps = ppnet_multi(images_test)
if prototype_name == 'support':
    logits = output[1]
    prototype_activations = max_similarities[1]
    prototype_activation_patterns = similarity_maps[1]
else:
    logits = output[0]
    prototype_activations = max_similarities[0]
    prototype_activation_patterns = similarity_maps[0]
...
log('Most activated 5 support/trivial prototypes of this image:')
...
for i in range(1, 6):   ######
    ...
    if prototype_name == 'support':
        log('last layer connection with predicted class: {0}'.format(
            ppnet.last_layer_support.weight[predicted_cls][sorted_indices_act[-i].item()]))
    else:
        log('last layer connection with predicted class: {0}'.format(
            ppnet.last_layer_trivial.weight[predicted_cls][sorted_indices_act[-i].item()]))
...
for i, c in enumerate(topk_classes.detach().cpu().numpy()):
    ...
    for j in reversed(sorted_indices_cls_act.detach().cpu().numpy()):
        ...
        if prototype_name == 'support':
            log('last layer connection: {0}'.format(ppnet.last_layer_support.weight[c][prototype_index]))
        else:
            log('last layer connection: {0}'.format(ppnet.last_layer_trivial.weight[c][prototype_index]))
